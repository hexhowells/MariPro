...
def uniform_mutation(evo, chromosomes, fitness_scores):
    """ Uniform mutation - modifies random genes by small amounts
...
def random_mutation(evo, chromosomes, fitness_scores):
    """ Random mutation - changes random genes uniformly
    ...
    for chromosome in chromosomes:
        for param in chromosome.parameters():
            ...
            # Mutate the selected weights.
            with torch.no_grad():
                param.flatten()[indices] = torch.randn(num_to_mutate) * 0.1
